    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.leave-types.update", [$leaveType->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.leaveType.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $leaveType->name) }}">
                @if($errors->has('name'))
