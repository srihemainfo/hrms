    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.nationalities.update", [$nationality->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.nationality.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $nationality->name) }}">
                @if($errors->has('name'))
