...
@section('content')
    ...
    <div class="card">
        <div class="card-header text-uppercase text-center">
            <p class='text-center  text-uppercase  rounded'> <strong> {{ trans('global.create') }} LAB Exam Schedule
                </strong></p>
        </div>
        ...
        <div class="card-body bg-light bg-gradient-primary">
            ...
            <div class="row">
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    ...
                    <select class="form-control select2 {{ $errors->has('accademicYear') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($AcademicYear as $id => $entry)
                            <option
                                value="{{ $id }}"{{ ($labExamTimetable->accademicYear ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12  float-right">
                    ...
                    <select class="form-control select2 {{ $errors->has('semesterType') ? 'is-invalid' : '' }}"
                        ...
                        <option value="ODD" {{ ($labExamTimetable->semesterType ?? '') == 'ODD' ? 'selected' : '' }}>ODD
                        </option>
                        <option value="EVEN" {{ ($labExamTimetable->semesterType ?? '') == 'EVEN' ? 'selected' : '' }}>
                            EVEN
                        </option>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    ...
                    <select class="form-control select2 {{ $errors->has('course_id') ? 'is-invalid' : '' }}" name="course"
                        ...
                        @foreach ($courses as $id => $entry)
                            <option value="{{ $id }}"
                                {{ ($labExamTimetable->course_id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    ...
                    <select class="form-control select2 {{ $errors->has('year') ? 'is-invalid' : '' }}" name="year"
                        ...
                        <option value="01" {{ ($labExamTimetable->year ?? '') == '01' ? 'selected' : '' }}>I
                        </option>
                        <option value="02" {{ ($labExamTimetable->year ?? '') == '02' ? 'selected' : '' }}>II
                        </option>
                        <option value="03" {{ ($labExamTimetable->year ?? '') == '03' ? 'selected' : '' }}>III
                        </option>
                        <option value="04" {{ ($labExamTimetable->year ?? '') == '04' ? 'selected' : '' }}>IV
                        </option>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    ...
                    <select class="form-control select2" name="semester" id="semester" required>
                        @foreach ($semester as $id => $entry)
                            <option value="{{ $id }}"
                                {{ ($labExamTimetable->semester ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    <label class="required d-block" for="MarkType">Mark Type</label>
                    <select class="form-control select2 {{ $errors->has('MarkType') ? 'is-invalid' : '' }}" name="MarkType"
                        id="MarkType" required>
                        <option value="">Please Select</option>
                        @foreach ($MarkType as $entry)
                            <option value="{{ $entry['name'] }}"
                                {{ ($labExamTimetable->MarkType ?? '') == $entry['name'] ? 'selected' : '' }}>
                                {{ $entry['name'] }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    ...
                    <select class="form-control select2" name="section" id="sections">
                        <option value="">Please Select</option>
                        <option value="A" {{ ($labExamTimetable->section ?? '') == 'A' ? 'selected' : '' }}>A</option>
                        <option value="B" {{ ($labExamTimetable->section ?? '') == 'B' ? 'selected' : '' }}>B</option>
                        <option value="C" {{ ($labExamTimetable->section ?? '') == 'C' ? 'selected' : '' }}>C</option>


                    </select>
                </div>
                <div class="form-group col-xl-3 col-lg-3 col-md-3 col-sm-4 col-12 ">
                    <label for="due_date" class="required d-block">Due Date</label>
                    <input type="date" class="form-control date" name="due_date" id="due_date"
                        value="{{ $labExamTimetable->due_date ?? '' }}">
            ...
            <div id='subject_head' style='display:block'>
                <div class="table-responsive">
                    <table class="table table-bordered text-center table-striped table-hover mt-3">
                        <thead>
                            <tr class='text-uppercase ' disabled>
                        ...
                        <tbody id="tabledata">
                            @php
                                $i = 0;
                            @endphp
                            @foreach ($labExamTimetable->newsubject as $id => $subjectA)
            ...
            <div class='text-center m-2'>
                <button class="btn btn-primary pl-5 pr-5 " id='submit' onclick="submit()" type="button">
                    {{ trans('global.save') }}
                </button>
            </div>
        ...
        <input type="hidden" name="examName" id="examName" value="{{ $labExamTimetable->exam_name ?? '' }}">
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const $old_accademicYear = $academicYearSelect.val();
            const $old_semesterType = $semesterType.val();
            const $old_course_id = $courseSelect.val();
            const $old_year = $year.val();
            const $old_semester = $semesterSelect.val();
            const $old_markType = $MarkType.val();
            ...
            function attemptAjaxCall() {
                ...
                if ($academicYearSelect.val() !== "" && $semesterType.val() != '' && $courseSelect.val() !== "" &&
            ...
            function makeAjaxCall() {
                ...
                newObj = []
                $.ajax({
                    url: "{{ route('admin.lab_examTimetable_edit.Subject_get') }}",
                    type: "POST",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                        'accademicYear': $academicYearSelect.val(),
                        ...
                        'old_accademicYear': $old_accademicYear,
                        'old_semesterType': $old_semesterType,
                        'old_semester': $old_semester,
                        'old_course_id': $old_course_id,
                        'old_year': $old_year,
                        'labExam': $labExam,
                        'old_markType': $old_markType,
                        'old_section': $old_section,
                        'id': {{ $labExamTimetable->id }},
                    },
                    success: function(response) {
                        if (response.exam_status != 'Already Done') {
                            newObj = [];
                            if (response.status == 'fail') {
                                ...
                                if (response.get_section.length > 0) {
                                    ...
                                    if (get_subjects.length > 0) {
                                        ...
                                        let exam = '';
                                        var exam_length = response.examName.length;
                                        if (exam_length > 0) {
                                            exam += ' <option value="">Select MarkTpe</option>';
                                            let got_examName = response.examName;
                                            for (let i = 0; i < got_examName.length; i++) {
                                                exam +=
                                                    `<option style="color:blue;" ${got_examName[i].name == $MarkType.val() ?? '' ? 'selected' : '' }  value="${got_examName[i].name}"> ${got_examName[i].name}</option>`;
                                            }

                                        } else {
                                            exam +=
                                                ' <option value=""> MarkType Not Available</option>';
                            ...
                            $tabledata.html(
                                '<th colspan="4" class="text-center"><strong >Select MarkType Already Created For Selected Section</strong></th>'
                            );
                            $tabledata.show();
                            $('#submit').hide();
                            $('#addSubject').hide();
        ...
        function submit() {
            ...
            if (length_check > 0) {
                if (!isEmpty) {
                    $.ajax({
                        url: "{{ route('admin.lab_markController.update', [$labExamTimetable->id]) }}",
                        type: "POST",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
