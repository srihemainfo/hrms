...
@section('content')
    ...
    @endphp
    <a class="btn btn-default" style="margin-bottom:17px;" onclick="goBack()">
        Back
    </a>
    <form action="{{ route('admin.subjectRegistration.store') }}" method="post" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="user_name_id" id="user_name_id" value="{{ $user_name_id }}">
    ...
    <script>
        $(document).ready(function() {
            $('.subject-checkbox').on('change', function() {
                var limit1 = parseInt($(this).data('limit'));
                var checkedCount = $('.subject-checkbox:checked').length;
                $('.subject-checkbox').not(':checked').prop('disabled', checkedCount >= limit1);
            });
            $('.subject-checkbox-2').on('change', function() {
                var limit2 = parseInt($(this).data('limit'));
                var checkedCount = $('.subject-checkbox-2:checked').length;
                $('.subject-checkbox-2').not(':checked').prop('disabled', checkedCount >= limit2);
            });
            $('.subject-checkbox-3').on('change', function() {
                var limit3 = parseInt($(this).data('limit'));
                var checkedCount = $('.subject-checkbox-3:checked').length;
                $('.subject-checkbox-3').not(':checked').prop('disabled', checkedCount >= limit3);
            });
        });
