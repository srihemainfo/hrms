...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <h6 class="text-center">Subject Wise Report</h6>
        </div>
        <div class="card-body">
            <form>
                <div class="row">
    ...
    <div class="card" style="display:none;" id="card">
        <div class="card-body">
            <table class=" table table-bordered table-striped table-hover datatable datatable-SubjectWise text-center">
                <thead>
                    <tr>
                        <th>
                            S.No
...
@section('scripts')
    <script>
        ...
        function get_data() {
            ...
            if (department != '' && department != 9 && department != 10 && course != '' && academic_year != '' && semester != '' && section != '') {
                $("#card").show();
                let default_row = `<tr><td colspan="10">Loading...</td></tr>`;
                $("#tbody").html(default_row);

                $.ajax({
                    url: '{{ route('admin.subject-wise-subject-registration.search') }}',
                    type: "POST",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    ...
                    success: function(response) {
                        ...
                        if (response.data) {
                            ...
                            if (len > 0) {
                                for (let a = 0; a < len; a++) {
                                    var enroll = list[a]['enroll'];
                                    var subject = list[a]['subject'];
                                    let url =
                                        "{{ route('admin.subject-wise-subject-registration.show', ['enroll' => ':enroll', 'subject' => ':subject']) }}";
                                    url = url.replace(':enroll', enroll);
                                    ...
                                    row += `<tr>
                                            ...
                                            <td>${list[a]['subject_category']}</td>
                                            <td>${list[a]['subject_code']}</td>
                                            <td>${list[a]['subject_title']}</td>
                        ...
                        let table = $('.datatable-SubjectWise').DataTable(dtOverrideGlobals);
