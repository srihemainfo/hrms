...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.toolsCourse.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.tools-courses.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('short_form') ? 'is-invalid' : '' }}" type="text"
                        name="short_form" id="short_form" value="{{ old('short_form', '') }}" required>
