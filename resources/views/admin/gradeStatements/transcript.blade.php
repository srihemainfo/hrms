...
@section('content')
                ...
                <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-12">
                    <div class="form-group text-right" id="action_div">
                        <button class="enroll_generate_bn bg-primary" style="margin-top:32px;"
                            onclick="searchTranscript()">Search</button>
                        @can('transcript_generate_access')
    ...
    <div class="card">
        <div class="card-body">
            <table
                class="table table-bordered table-striped table-hover ajaxTable datatable datatable-Transcript text-center">
                <thead>
                    <tr>
                        <th>
                            S.No
...
@section('scripts')
    <script>
        ...
        function callAjax() {
            ...
            if ($.fn.DataTable.isDataTable('.datatable-Transcript')) {
                $('.datatable-Transcript').DataTable().destroy();
            }
            let dtOverrideGlobals = {
                buttons: dtButtons,
                retrieve: true,
                aaSorting: [],
                ajax: "{{ route('admin.transcript.index') }}",
                columns: [{
                        data: 'id',
                        name: 'id'
                    },
                    ...
                    },
                    {
                        ...
                        render: function(response) {
                            ...
                            return `<a class="btn btn-sm btn-success" target="_blank" href="{{ url('admin/transcript/pdf') }}/${data.batch}/${data.course}/${data.regulation}"> View PDF</a>`;
                        },
                        type: 'html'
                    },
                ],
        ...
        function searchTranscript() {
            if ($("#regulation").val() == '') {
                ...
                let dtOverrideGlobals = {
                    ...
                    aaSorting: [],
                    ajax: {
                        url: "{{ route('admin.transcript.search') }}",
                        method: "POST",
                        headers: {
        ...
        function checkGeneration() {
            if ($("#regulation").val() == '') {
                ...
                $("#action_div").hide();
                $.ajax({
                    url: "{{ route('admin.transcript.check-generation') }}",
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
        ...
        function reGenerate() {
            ...
            Swal.fire({
                title: "Transcript Already Generated for this Batch !",
                text: "",
                icon: "warning",
                ...
                if (result.value) {
                    ...
                    $("#action_div").hide();
                    $.ajax({
                        url: "{{ route('admin.transcript.generation') }}",
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
