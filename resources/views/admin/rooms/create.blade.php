        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.rooms.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('block') ? 'is-invalid' : '' }}" name="block_id"
                        ...
                        @foreach ($blocks as $id => $entry)
                            <option value="{{ $id }}" {{ old('block_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('block'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('room_no') ? 'is-invalid' : '' }}" type="number"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('no_of_class_seats') ? 'is-invalid' : '' }}" type="number"
                        name="no_of_class_seats" id="no_of_class_seats" value="{{ old('no_of_class_seats', '') }}" step="1">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('no_of_exam_seats') ? 'is-invalid' : '' }}" type="number"
                        name="no_of_exam_seats" id="no_of_exam_seats" value="{{ old('no_of_exam_seats', '') }}" step="1">
