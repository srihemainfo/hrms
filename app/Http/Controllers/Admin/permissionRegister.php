...
class permissionRegister extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.permissionRegister.index', compact('staff', 'department'));
        ...
        if ($request) {
            ...
            if (auth()->user()->roles[0]->id != 42) {
                if ($startDate != null && $endDate != null) {
                    ...
                    $results = PermissionRequest::where('dept', '=', $dept)
                        ->where(['status' => 2])
                        ->get();
                }
                ...
                foreach ($results as $data) {
                    ...
                    $get_staff = TeachingStaff::where(['user_name_id' => $data->user_name_id])->select('StaffCode')->first();
                    if ($get_staff == '') {
